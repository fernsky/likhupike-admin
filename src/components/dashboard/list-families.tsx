...
export default function ListFamilies({ user }: { user: User }) {
  ...
  if (familiesError) {
    return (
      <Alert variant="destructive" className="m-4">
        <AlertDescription>
          {familiesError?.message || "An error occurred"}
        </AlertDescription>
      </Alert>
    );
  }
    ...
    },
    {
      ...
      cell: ({ row }: { row: any }) => (
        <div className="flex gap-2">
          <Link href={`/family/${row.original.id}`}>
      ...
      <div className="rounded-lg border bg-card shadow-sm">
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <StatCard title="Total Families" value={data?.items.length || 0} />
          </div>
